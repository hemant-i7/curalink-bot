...
export async function POST(request: Request) {
  try {
    ...
    console.log('Initiating health consultation call to:', phoneNumber, 'with assistant:', assistantId);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to initiate health consultation call' },
      { status: 500 }
    );
  }
}
  ...
  return NextResponse.json(
    { 
      ...
      debug: {
        ...
        assistantIdLength: process.env.NEXT_PUBLIC_VAPI_GENERATE_ASSISTANT_ID?.length || 0,
      }
    },
    { status: 200 }
  );
