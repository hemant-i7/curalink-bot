...
export async function POST(request: Request) {
  try {
    ...
    if (!PRIVATE_API_KEY) {
      throw new Error(
        "VAPI Private API Key is not configured in environment variables"
      );
    }

    if (!PHONE_NUMBER_ID) {
      throw new Error(
        "VAPI Phone Number ID is not configured in environment variables"
      );
    }

    const body = await request.json();
    const { phoneNumber: rawPhoneNumber, assistantId: customAssistantId } =
      body;
    ...
    console.log(
      "Initiating health consultation call to:",
      phoneNumber,
      "with assistant:",
      assistantId
    );
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to initiate health consultation call",
      },
      { status: 500 }
    );
  }
...
export async function GET() {
  ...
  return NextResponse.json(
    {
      message: "Health consultation call API is running",
      debug: {
        hasPrivateKey: !!process.env.VAPI_PRIVATE_API_KEY,
        hasPhoneNumberId: !!process.env.VAPI_PHONE_NUMBER_ID,
        hasAssistantId: !!process.env.NEXT_PUBLIC_VAPI_GENERATE_ASSISTANT_ID,
        privateKeyLength: process.env.VAPI_PRIVATE_API_KEY?.length || 0,
        phoneNumberIdLength: process.env.VAPI_PHONE_NUMBER_ID?.length || 0,
        assistantIdLength:
          process.env.NEXT_PUBLIC_VAPI_GENERATE_ASSISTANT_ID?.length || 0,
      },
    },
    { status: 200 }
  );
